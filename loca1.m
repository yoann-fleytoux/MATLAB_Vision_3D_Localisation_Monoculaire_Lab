...
%floor : rounds each element of X to the nearest integer less than or equal to that element.
%point 1 : u1->y, v1->x 
%point 2 : u2->y, v2->x 
for p=1:N
    ...
    vector_origin_pt1= [(u1(p) - u0_1)/alphaU_1, (v1(p) - v0_1)/alphaV_1 , 1];%l2
    
    %C = cross(A,B) returns the cross product of A and B.
    %If A and B are vectors, then they must have a length of 3.
    normale = cross(vector_origin_pt1, vector_segment) / norm(cross(vector_origin_pt1, vector_segment))
...
while (1),
    ...
    % matPos = 
     ...
     dmatPos = TransFromParam(dsoluL);% les chagements à faire sur les param
     matPos = dmatPos * matPos;%applique les transformations sur les param
    
    % *******************************************************************
   
    soluL=ParamFromTrans(matPos);%matrice de passage repère monde à repère caméra
    for p=1:N
        [App3d_mod(p,1) App3d_mod(p,3) App3d_mod(p,5)]=TransPoint(matPos,App3d_ini(p,1),App3d_ini(p,3),App3d_ini(p,5));
        [App3d_mod(p,2),App3d_mod(p,4),App3d_mod(p,6)]=TransPoint(matPos,App3d_ini(p,2),App3d_ini(p,4),App3d_ini(p,6));
    end;
